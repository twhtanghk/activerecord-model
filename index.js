// Generated by CoffeeScript 1.12.7
(function() {
  var Promise, _, async, await, http, ref, stampit, util;

  _ = require('lodash');

  ref = require('asyncawait'), async = ref.async, await = ref.await;

  Promise = require('bluebird');

  stampit = require('stampit');

  http = Promise.promisifyAll(require('needle'));

  util = require('util');

  module.exports = {
    token: async(function(opts) {
      var body, client, data, scope, url, user;
      url = opts.url, client = opts.client, user = opts.user, scope = opts.scope;
      opts = {
        'Content-Type': 'application/x-www-form-urlencoded',
        username: client.id,
        password: client.secret
      };
      data = {};
      if (user != null) {
        data = {
          grant_type: 'password',
          username: user.id,
          password: user.secret,
          scope: scope.join(' ')
        };
      } else {
        data = {
          grant_type: 'client_credentials'
        };
      }
      body = await(module.exports.api().post(url.token, data, opts)).body;
      if (body.error != null) {
        throw new Error(body.error);
      }
      return body.access_token;
    }),
    verify: function(opts) {
      var authScope, res, result, scope, token, url;
      url = opts.url, scope = opts.scope, token = opts.token;
      opts = {
        headers: {
          Authorization: "Bearer " + token
        }
      };
      res = await(module.exports.api().get(url.verify, null, opts));
      if (res.statusCode !== 200) {
        throw new Error(res.body);
      }
      authScope = res.body.scope.split(' ');
      result = _.intersection(scope, authScope);
      if (result.length !== scope.length) {
        throw new Error("Unauthorizated access to " + scope);
      }
      return res.body;
    },
    validToken: async(function(getToken, opts) {
      var err, token, verified;
      token = null;
      while (true) {
        try {
          verified = await(module.exports.verify(_.extend({
            token: token
          }, opts)));
          return token;
        } catch (error) {
          err = error;
          token = await(getToken());
        }
      }
    }),
    api: function() {
      var stamp;
      return stamp = stampit().compose(http).statics({
        get: async(function(url, data, opts) {
          if (opts == null) {
            opts = {};
          }
          if (data != null) {
            if (opts.headers == null) {
              opts.headers = {};
            }
            _.extend(opts.headers, {
              'Content-Type': 'application/json',
              'x-http-method-override': 'get'
            });
            return await(http.postAsync(url, data, opts));
          } else {
            return await(http.getAsync(url, opts));
          }
        }),
        put: async(function(url, data, opts) {
          return await(http.putAsync(url, data, opts));
        }),
        post: async(function(url, data, opts) {
          return await(http.postAsync(url, data, opts));
        }),
        'delete': async(function(url, data, opts) {
          return await(http.deleteAsync(url, data, opts));
        })
      });
    },
    authApi: function(token) {
      var api, opts;
      opts = async(function() {
        return {
          rejectUnauthorized: false,
          headers: {
            Authorization: "Bearer " + (await(token()))
          }
        };
      });
      api = module.exports.api();
      return stampit().compose(api).statics({
        get: async(function(url, data) {
          return await(api.get(url, await(opts())));
        }),
        put: async(function(url, data) {
          return await(api.put(url, data, await(opts())));
        }),
        post: async(function(url, data) {
          return await(api.post(url, data, await(opts())));
        }),
        'delete': async(function(url) {
          return await(api.delele(url, null, await(opts())));
        })
      });
    },
    model: function(baseUrl) {
      var stamp;
      return stamp = stampit().init(function(props) {
        return _.extend(this, this.parse(props));
      }).methods({
        getStamp: function() {
          return stamp;
        },
        isNew: function() {
          return this[this.getStamp().idAttribute] == null;
        },
        parse: function(data) {
          if (data == null) {
            data = {};
          }
          return data;
        },
        fetch: async(function() {
          var res;
          res = await(stamp.api.get(stamp.url(this[this.getStamp().idAttribute])));
          return _.extend(this, this.parse(res));
        }),
        save: async(function(values) {
          var res;
          if (values == null) {
            values = {};
          }
          _.extend(this, values);
          if (this.isNew()) {
            res = await(stamp.api.post(stamp.url(), this));
          } else {
            res = await(stamp.api.put(stamp.url(this[this.getStamp().idAttribute]), this));
          }
          return _.extend(this, this.parse(res));
        }),
        destroy: function() {
          return await(stamp.api["delete"](stamp.url(this[this.getStamp().idAttribute])));
        }
      }).statics({
        idAttribute: 'id',
        baseUrl: baseUrl,
        api: module.exports.api(),
        use: function(api) {
          return this.api = api;
        },
        url: function(id) {
          var path, ret, url;
          if (id == null) {
            id = '.';
          }
          url = require('url');
          path = require('path');
          ret = url.parse(this.baseUrl);
          ret.pathname = path.join(ret.pathname, id);
          return url.format(ret);
        },
        fetchOne: async(function(id) {
          var props;
          props = {};
          props[this.idAttribute] = id;
          return await(this(props).fetch());
        }),
        fetchAll: async(function(data) {
          if (data == null) {
            data = null;
          }
          return await(this.api.get(stamp.url));
        })
      });
    }
  };

}).call(this);
